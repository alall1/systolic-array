module pe #(
    ...
    //input logic capture,                            // pulse that copies current acc -> shadow_out
    //input logic shift_en,                           // pulse that enables shifting (shadow_in -> shadow_out)
    //input logic signed [ACC_WIDTH-1:0] shadow_in,   // the shadow buffer value shifted down by the top neighbor, copied to shadow_out while shift_en
    //output logic signed [ACC_WIDTH-1:0] shadow_out  // the actual shadow buffer register of this PE, shifts to bottom neighbor while shift_en
);
