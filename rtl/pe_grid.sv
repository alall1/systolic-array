module pe_grid #(
    ...
    input logic a_valid [0:N-1],                            // the current valid signal column of matrix A entering the grid
    input logic b_valid [0:N-1],                            // the current valid signal column of matrix B entering the grid
...
logic a_valid_bus [0:N-1][0:N];                 // bus of all horizontal valid signal connections between PEs, similar to a_bus (a_valid propagates with a's value)
logic b_valid_bus [0:N][0:N-1];                 // bus of all vertical valid signal connections between PEs, similar to b_bus (b_valid propagates with b's value)
