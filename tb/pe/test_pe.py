...
async def step(dut, in_a: int, in_b: int, in_valid: int, data_width: int):
...
async def test_single_mac(dut):
    ...
    a, b = 7, -3
    await step(dut, a, b, in_valid=1, data_width=data_width)
...
async def test_operand_propagation(dut):
    """out_a/out_b are the previous cycle's inputs; out_valid tracks in_valid delayed one cycle"""
    data_width, acc_width = get_params(dut)
    model = PEModel(data_width, acc_width)
...
async def test_bubble_holds_acc(dut):
    """
    Accumulate, insert an in_valid=0 bubble, then resume. Across the bubble:
    ...
    await step(dut, 99, 99, in_valid=0, data_width=data_width)
...
async def test_random_regression(dut):
    ...
    for i in range(200):
        ...
        await step(dut, a, b, in_valid=v, data_width=data_width)
        model.step(a, b, in_valid=v)
